...
async function run(): Promise<void> {
    try {
        ...
        const sqlScript = <cm.ISqlPlusInput>{
            type: inputType,
            content: inputType === cm.InputType.File ? tl.getPathInput('SQLScriptPath', true, true) : tl.getInput('SQLScript', true),
            substitutionVariables: cm.parseSubstitutionVariables(tl.getInput('Parameters'))
        };
